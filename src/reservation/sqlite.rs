...
use super::{
    models::{Reservation, ReservationDetail, ReservationQuery},
...
impl ReservationRepo for SqliteReservationRepo {
    async fn find_by_query(&self, query: ReservationQuery) -> Vec<ReservationDetail> {
        todo!()
    }

    async fn find_by_book_ref(&self, book_ref: &str) -> Option<ReservationDetail> {
